...
// --------------------------
// (3) 載入題庫 (HRMquestion.json)
// --------------------------
async function loadQuestions() {
  try {
    const response = await fetch("HRMquestion.json");
    if (!response.ok) {
...
function getQuestionOptions(q) {
  return ["A", "B", "C", "D", "E", "F"].filter(letter => q && q[letter] !== undefined && q[letter] !== null && String(q[letter]).trim() !== "");
}

function renderOptionInputs(q, index, userAnswer) {
  return getQuestionOptions(q).map(letter => `
          <label><input type="radio" name="q${index}" value="${letter}" ${userAnswer === letter ? 'checked' : ''}><span> ${letter}. ${q[letter]}</span></label>`).join("");
}

function renderOptionTexts(q) {
  return getQuestionOptions(q).map(letter => `
      <p>${letter}. ${q[letter]}</p>`).join("");
}
...
function renderQuestions() {
  ...
  selectedQuestions.forEach((id, index) => {
    ...
    questionDiv.innerHTML = `
      ...
      <div class="options">
          ${renderOptionInputs(q, index, userAnswer)}
      </div>
